...
updateGrid : (Coord -> Int -> Int) -> Grid -> Grid
updateGrid mapper grid =
    Array.indexedMap (\y row -> Array.indexedMap (\x value -> mapper ( x, y ) value) row) grid


instructGrid : (String -> Int -> Int) -> Instruction -> Grid -> Grid
instructGrid opfunc instruction grid =
    let
        { op, from, to } =
            instruction
    in
        updateGrid (\coord val -> ifBetween from to coord (opfunc op) val) grid


ifBetween : Coord -> Coord -> Coord -> (Int -> Int) -> Int -> Int
ifBetween from to coord func val =
    if Coords.isBetween from to coord then
        func val
    else
        val


sumBrightness : Grid -> Int
sumBrightness grid =
    Array.toList grid
...
type alias Instruction =
    { op : String, from : Coord, to : Coord }


instructionPattern =
    Regex.regex "(turn on|turn off|toggle) (\\d+),(\\d+) through (\\d+),(\\d+)"


instructionFromMatch : Regex.Match -> Instruction
instructionFromMatch match =
    let
        submatches =
            match.submatches |> List.map (Maybe.withDefault "")
        ...
        bx =
            submatches |> List.Extra.getAt 3 |> Maybe.withDefault "" |> String.toInt |> Result.withDefault 0

        by =
            submatches |> List.Extra.getAt 4 |> Maybe.withDefault "" |> String.toInt |> Result.withDefault 0
    in
        { op = op, from = ( ax, ay ), to = ( bx, by ) }


parseInstruction : String -> Instruction
parseInstruction s =
    Regex.find Regex.All instructionPattern s
        |> List.head
        |> Maybe.map instructionFromMatch
        |> Maybe.withDefault { op = "", from = ( 0, 0 ), to = ( 0, 0 ) }
...
part1 =
    let
        ...
        baseGrid =
            newGrid 1000
    in
        List.foldl instructGrid1 baseGrid instructions
            |> sumBrightness
...
part2 =
    let
        ...
        baseGrid =
            newGrid 1000
    in
        List.foldl instructGrid2 baseGrid instructions
            |> sumBrightness
