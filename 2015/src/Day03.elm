...
toCoords : List String -> List Coord
toCoords instructions =
    toCoordsInternal instructions Coords.origin []


toCoordsInternal : List String -> Coord -> List Coord -> List Coord
toCoordsInternal instructions coord coords =
    case instructions of
        next :: rest ->
            toCoordsInternal rest (nextCoord next coord) (coords ++ [ coord ])

        _ ->
            coords


nextCoord : String -> Coord -> Coord
nextCoord inst coord =
    if inst == "^" then
        Coords.add coord ( 0, 1 )
    else if inst == "v" then
        Coords.add coord ( 0, -1 )
    else if inst == "<" then
        Coords.add coord ( -1, 0 )
    else if inst == ">" then
        Coords.add coord ( 1, 0 )
    else
        Debug.crash "Where am i going?"
...
indexedFilter : (Int -> Bool) -> List a -> List a
indexedFilter predicate list =
    list
        |> List.indexedMap (,)
        |> List.filter (\v -> Tuple.first v |> predicate)
    ...
    let
        ...
        robosantaCoords =
            input
                ...
                |> toCoords
    in
        List.append santaCoords robosantaCoords
            |> List.Extra.unique
            |> List.length
